...
def vertical_solution(district):
    ...
    print("--- Running vertical solution with shared cables ---")
    ...
    add_branches(district, batteries, houses)

    i = 1
    print(f"VERTICAL SOLUTION FOUND in {i} iterations")
    print_usage(district)

def add_branches(district, batteries, houses):
    """
    ...
    for house in houses:
        ...
        district.add_branch(nearest_free_battery(house, batteries), house)
...
def print_usage(district):
    ...
    for battery in district.get_batteries():
        ...
        print(f"{battery.get_cable().path}")
